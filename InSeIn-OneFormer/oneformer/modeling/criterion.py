    ...
    def phyfea_loss(self, outputs, targets, indices, num_masks):
        ...
        with torch.no_grad():
             if self.dataset_used == 'Cityscapes':
                self.sublist = random.sample(invalid_pairs_cityscape, self.sublist_size)
             if self.dataset_used == 'ACDC':
                self.sublist = random.sample(invalid_pairs_acdc, self.sublist_size)
             if self.dataset_used == 'ADE20k':
                self.sublist = random.sample(invalid_list_ADE20k, self.sublist_size)

             self.phyfea.invalid_pair_list = self.sublist
